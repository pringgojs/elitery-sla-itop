    ...
    <div class="relative z-50 bg-white rounded-lg shadow dark:bg-gray-700 ">
        ...
        <div class="flex items-center justify-between p-4 rounded-t md:p-5 dark:border-gray-600">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                {{ $id ? 'Perbaruhi Jenis Kegiatan' : 'Tambah Jenis Kegiatan' }}
            </h3>
            ...
            <form wire:submit="store" class="space-y-4 md:space-y-6" autocomplete="off">
                ...
                <div>
                    <label for="color" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Warna Marker Peta</label>
                    <input type="color" wire:model="form.color" class="" value="{{ $form->color }}"
                        placeholder="">
                    <div>
                        @error('form.color')
                            <span class="text-red-500">{{ $message }}</span>
