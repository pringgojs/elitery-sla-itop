<div>
    @livewire('pages.semester-transaction.section.filter')
        ...
        <div class="space-y-4 mb-3">
            ...
            <div class="bg-white p-4 rounded-lg shadow grid grid-cols-12 items-center gap-4">
                    ...
                    <div class="flex -space-x-3">
                        @foreach ($item->details->take(3) as $detail)
                            <span data-tooltip-target="tooltip-default-{{ $detail->id }}"
                                class="inline-flex h-8 w-8 cursor-pointer border-white font-bold text-sm text-gray-500 border-2 items-center justify-center rounded-full bg-gray-200">{{ initials($detail->good->name ?? $detail->good_name) }}</span>
                            </span>
                            <div id="tooltip-default-{{ $detail->id }}" role="tooltip"
                                class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                {{ $detail->good->name ?? $detail->good_name }}
                                <div class="tooltip-arrow" data-popper-arrow></div>
                            </div>
                        @endforeach
                        {{-- <img class="w-8 h-8 rounded-full border-2 border-white" src="https://via.placeholder.com/32"
                            alt="Member 1">
                        <img class="w-8 h-8 rounded-full border-2 border-white" src="https://via.placeholder.com/32"
                            alt="Member 2">
                        <img class="w-8 h-8 rounded-full border-2 border-white" src="https://via.placeholder.com/32"
                ...
                <div class="col-span-3 flex items-center justify-end space-x-1">
                    @if (!$item->approved_by)
                        @can('transaksi.permohonan.pengadaan.barang.process')
                            <a href="{{ route('semester-transaction.preview', ['id' => $item->id]) }}" wire:navigate
                    ...
                    <a href="{{ route('semester-transaction.print', ['id' => $item->id]) }}" wire:navigate
                        class="flex shrink-0 justify-center items-center gap-2 size-[38px] text-sm font-medium rounded-lg bg-white p-2 text-gray-400 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                    ...
                    <div class="relative inline-block text-left">
                        ...
                        <div id="dropdown-more-{{ $item->id }}"
                            ...
                            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                                ...
                                @if (!$item->approved_by)
                                    @can('transaksi.permohonan.pengadaan.barang.edit')
                                        <li>
                                            <a href="{{ route('semester-transaction.edit', ['id' => $item->id]) }}"
                                                wire:navigate
                                                class="cursor-pointer group flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                                role="menuitem" tabindex="-1" id="menu-item-0">
                                    ...
                                    @endcan
                                    @can('transaksi.permohonan.pengadaan.barang.delete')
                                        <li>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm text-yellow-700">
                        ...
                        <a href="{{ route('semester-transaction.create') }}" wire:navigate
